...
const GamePage = () => {
  ...
  const screenshotsRequest = useApiRequest(() =>
    getGameDetailScreenshots(gameId)
  );
  ...
  return gameRequest.loading ||
    screenshotsRequest.loading ||
    !gameRequest.data ? (
    ...
    <main className="relative grow z-0 w-full bg-neu-1 dark:bg-neu1-10 pt-[30px] pb-[50px] px-[20px] overflow-auto animate-[fadeIn_0.5s_ease-out]">
      <div
        ...
        style={{
          backgroundImage: `url("${gameRequest.data.background_image}")`,
        }}
      ></div>
      <div
        ...
        style={{
          background:
            theme == "dark"
              ? "linear-gradient(rgba(31,41,51,.65), #1F2933)"
              : "linear-gradient(rgba(229,231,235,.65), #e5e7eb)",
        }}
      ></div>

      {
        <MainInfoSection
          game={gameRequest.data || null}
          screenshots={screenshotsRequest.data.results || null}
        ></MainInfoSection>
      }

      {
        <PlatformsSection
          platforms={gameRequest.data.platforms || null}
        ></PlatformsSection>
      }

      {<GenresSection genres={gameRequest.data.genres || null}></GenresSection>}

      {!!gameRequest.data.developers &&
        gameRequest.data.developers.length > 0 && (
          <DevelopersSection
            developers={gameRequest.data.developers}
          ></DevelopersSection>
        )}

      {!!gameRequest.data.metacritic && (
        <MetacriticSection
          score={gameRequest.data.metacritic}
          metacriticURL={gameRequest.data.metacritic_url}
        ></MetacriticSection>
      )}
    </main>
  );
