...
const GameCardSlideshow = props => {
	...
	const getGameImages = imageList => {
		...
		const images = imageList.map((x, i) => (
			<img
				alt=""
				loading="lazy"
				key={i}
				src={getMidCompressedImageURL(x.image)}
				style={{ display: hoverIndex === i ? 'block' : 'none' }}
				className={style}
			></img>
		));
		return images;
	...
	return (
		<div
			className="max-h-[200px] min-h-[200px] relative rounded-tl-lg rounded-tr-lg overflow-hidden"
			style={props.style}
			data-testid="game-card-slideshow"
		>
			{imageList}
			<div
			...
			>
				{new Array(imageList.length).fill().map((x, i) => (
					<div key={i} onMouseEnter={() => setHoverIndex(i)} className="grow flex z-20">
						<span
							className={`mt-auto mx-auto bg-white w-[60%] mb-[10px] h-[3px] ${
								hoverIndex === i ? 'opacity-80' : 'opacity-30'
							} rounded-[50px]`}
						></span>
					</div>
				))}
