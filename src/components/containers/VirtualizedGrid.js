...
const VirtualizedGrid = (props) => {
    ...
    return (
        <div ref={gridParentElement} className="flex flex-col overflow-auto" onScroll={handleScroll}>
            <div ref={header} className='box-border'>{props.header || ''}</div>
            <div ref={ghostGrid} style={{ rowGap: props.gapX, columnGap: props.gapY, gridTemplateColumns: `repeat(auto-fit, minmax(min(${props.columnWidth}px, 100%), 1fr))`}} className={`grid w-full block`}>{getGhostGridContent()}</div>
            <div style={{ minHeight: getHeight() || 0 }} className='relative w-full overflow-hidden'>
                {getVisibleElements()}
            </div>
            <div ref={footer} className='box-border'>{props.footer || ''}</div>
        </div>
    );
};
